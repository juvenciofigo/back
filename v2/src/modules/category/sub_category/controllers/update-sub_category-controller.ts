...
export async function updateSub_categoryController(req: Request, res: Response, next: NextFunction) {
    const { sub_categoryName, availability } = req.body;
    const subCategoryId = req.params.subCategoryId || "";
    const sub_categoryId = req.params.sub_categoryId || "";

    try {
        const subCategory = await makeUpdateSub_category().execute({ sub_categoryName, availability, subCategoryId, sub_categoryId });
